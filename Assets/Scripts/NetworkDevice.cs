...
public class NetworkDevice : MonoBehaviour
{
    ...
    public void SendFrame(NetworkFrame frame, Switch targetSwitch)
    {
        ...
        {
            Debug.Log($"[{Name}] Отправка кадра от {frame.SourceMac} к {frame.DestinationMac} (VLAN {frame.VlanId})");
    ...
    {
        Debug.Log($"[{Name}] Получен кадр от {frame.SourceMac}, данные: \"{frame.Payload}\" (VLAN {frame.VlanId})");
    }
}